# Initial Permutation Table: Table of 64 bits at initial level 
...
# Key Shifts 
RotateLeft = [1,1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1]

# Read text from a file for use with plaintext and key files
def fileInput(file):
...
def HexFunc(arr): 
    hex_arr = []
    for i in range(0,8):
        hex_arr.append(arr[8*i]*128 + arr[8*i + 1]*64 + arr[8*i + 2]*32 + arr[8*i + 3]*16 + arr[8*i + 4]*8 + arr[8*i + 5]*4 + arr[8*i + 6]*2 + arr[8*i + 7])
    hex_arr = [hex(x) for x in hex_arr]
    return hex_arr 

# Initial Permutation replacement of plaintext
# Divide into left and right substrings
def IP_Transpose(plaintext):
    LR = []
    for i in IP: 
...
def IP_InverseTranspose(LR):
    temp = []
    for i in FP:
        temp.append(LR[i-1])
    return temp

# Expand Right plaintext to 48 bits
def R_expand(R): 
    new_R = []
    for i in E: 
        new_R.append(R[i-1])
...
def xor(a, b): 
    ...
    for i in range(0, len(a)):
        product.append(int(a[i])^int(b[i]))
    return product

# Substitution step
def S_sub(xor_product):
    sub = []
    for i in range(0,8): 
        temp = xor_product[i*6:i*6+5]
        row = temp[0]*2+temp[-1]
        col = temp[1]*8+temp[2]*4+temp[3]*2+temp[4]
        sub.append('{:04b}'.format(S[i][row][col]))
    sub = ''.join(sub)
    return sub

# Replace substitution with P
def P_transpose(sub_product):
    permutation = []
    for i in Perm: 
        permutation.append(int(sub_product[i-1]))
    return permutation

# F function
def F(right, key): 
    new_right = R_expand(right)
    xor_right_key = xor(new_right, key)
    sub = S_sub(xor_right_key)
    permutation = P_transpose(sub)
    return permutation

# Key Transpose for key option 1
def Key_Transpose(key):
    LR = [] 
    for i in PC_1: 
        LR.append(int(key[i-1]))
    L = LR[:28]
    R = LR[28:]
    return L, R

# Rotate Key to the Left
def Key_RotateLeft(key, pos):
    new_key = key[pos:]
    for i in range(0,pos):
        new_key.append(key[i])
    return new_key

# Compress Key with key option 2
def Compress_Key(L, R): 
    key = L + R
    new_key = []
    for i in PC_2: 
...
def DES_encrypt(file, key, outputFile): 
    ...
    for i in range(0,15):
        ...
        R = xor(L, permutation)
        L = old_R
    permutation = F(R, K[15])
    L = xor(L, permutation)
    P_inverse = IP_InverseTranspose(L + R)
    Cipher = HexFunc(P_inverse)
...
def DES_decrypt(file, key, outputFile):
